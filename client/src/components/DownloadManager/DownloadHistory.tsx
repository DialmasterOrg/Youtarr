                      ...
                      >
                        ...
                        {job.data?.videos?.length > 0 && (
                          <>
                            ...
                            <Popper open={Boolean(anchorEl[job.id])} anchorEl={anchorEl[job.id]}>
<ClickAwayListener onClickAway={() => setAnchorEl({ ...anchorEl, [job.id]: null })}>
<div style={{
  padding: "8px",
  ...
  <Typography sx={{ padding: 2 }}>
    {job.data?.videos?.map((video: any) => (
      <p key={video.youtubeId}>
        {video.youTubeChannelName} - {video.youTubeVideoName}
      </p>
    ))}
  </Typography>
</div>
