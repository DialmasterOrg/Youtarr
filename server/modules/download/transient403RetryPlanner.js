...
function planAutoRetry({
  ...
  const retryVideos = failedVideosList
    ...
    .filter((video) =>
      isTransient403Failure(video, { httpForbiddenDetected }) ||
      isCookieVideoUnavailableFailure(video, { cookiesEnabled })
    )
    .map((video) => {
      const anonymousRetry = isCookieVideoUnavailableFailure(
        video,
        { cookiesEnabled }
      );
